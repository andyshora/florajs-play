    ...
    middleware: [async (req, res, next) => {
      ...
      try {
        ...
        if (pathname === '/css/main.min.css') {
          contents = await fs.readFile('./app/css/main.css');
          output = await compile.css(contents, { map: true });
          res.setHeader('Content-Type', 'text/css');
          res.end(output);
        } else if (pathname === '/js/main.min.js' || pathname === '/js/main.js') {
          output = await compile.js({debug: true});
          res.setHeader('Content-Type', 'application/javascript');
          res.end(output);
        } else {
          const filename = pathname === '/' ? './app/index.md' : './app' + pathname + '.md';
